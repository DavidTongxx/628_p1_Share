...
#First 50 days#
```{r}
air_0_50 <- air_0[air_0$Days <51,]

p_50_v1_line<-ggplot(data = air_0_50, 
                aes(x=Days,y = Value,color = Site))+ 
                geom_line() + geom_point() +
                geom_smooth()+ ggtitle("Hour 0")
#no CI

p_50_v2_line<-ggplot(data = air_0_50, 
                aes(x=Days,y = Value,color = Site))+ 
                geom_line() + geom_point() +
                geom_smooth(se = FALSE)+ ggtitle("Hour 0 (no CI)")
#no line
p_50_v1_noline<-ggplot(data = air_0_50, 
                aes(x=Days,y = Value,color = Site))+ 
                geom_point(aes(shape = Site ) )+
                geom_smooth(se = FALSE)+ ggtitle("Hour 0 (no line)")


p_50_v2_noline<-ggplot(data = air_0_50, 
                aes(x=Days,y = Value,color = Site))+ 
                geom_point(aes(shape = Site ) )+
                geom_smooth(se = FALSE)+ facet_wrap(~Site)+
                ggtitle("Hour 0 (separated)")

grid.arrange(p_50_v1_line,p_50_v2_line,p_50_v1_noline, p_50_v2_noline, ncol=2,nrow=2)
